...
class BaseApi {
	...
	async postFormData(endpoint: string, formData: FormData) {
		...
		const response = await this.request.post(endpoint, { multipart: formData });
	...
	async put(endpoint: string, data?: any) {
		...
		const response = await this.request.put(endpoint, data !== undefined ? { data } : undefined);
	...
	async patch(endpoint: string, data: any) {
		logger.info(`Sending PATCH request to ${endpoint} with data: ${JSON.stringify(data)}`);
		const response = await this.request.patch(endpoint, { data });
		if (response.ok()) {
			logger.info(`PATCH request to ${endpoint} successful`);
		} else {
			logger.error(`PATCH request to ${endpoint} failed with status code ${response.status()}`);
		}
		return response;
	}
