...
export default class GeolocationUtils {
	...
	static computeDistance = (lat1: number, lon1: number, lat2: number, lon2: number) => {
		...
		const a = Math.sin(dLat / 2) * Math.sin(dLat / 2) +
			Math.cos(GeolocationUtils.toRadians(lat1)) * Math.cos(GeolocationUtils.toRadians(lat2)) *
		...
		if (permission === RESULTS.GRANTED) {
			const watchId = Geolocation.watchPosition(
				...
				(error) => {
					...
					Alert.alert('Encountered an error while getting location.');
				},
			...
			Alert.alert('Permission denied', 'Without your permission, Stadspassen can not access your location.', [
				{
					text: 'Try Again',
