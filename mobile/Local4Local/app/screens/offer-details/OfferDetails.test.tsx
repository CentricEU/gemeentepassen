...
describe('OfferDetails', () => {
			...
			<NavigationContainer>
				<RefreshProvider>
					<LocationContext.Provider value={locationMock}>
						<OfferDetails route={{ params: { offerId: 1 } }} navigation={{}} />
	...
	it('should log an error if offer fails to fetch in getFullOffer', async () => {
		const consoleMock = jest.spyOn(global.console, 'error').mockImplementation(jest.fn());
	...
	describe('frequency of use messages', () => {
		...
		it.each(params)('%s should result in the %s error', async (restriction, error) => {
			const offer = {
				...offerMock,
				restrictions: {
			...
			(OfferService.getFullOffer as jest.Mock).mockResolvedValue(offer);
			offerMock.lastOfferTransaction.usageTime = new Date().toISOString();
	...
	describe('useOffer', () => {
		it('should claim the selected offer', async () => {
			...
			expect(OfferService.useOffer).toHaveBeenCalledWith(new UseOfferDto(offerMock.id, expect.anything()));
		});
	});
	...
	it('should show pass expired message when API returns 40056', async () => {
		(OfferService.getFullOffer as jest.Mock).mockResolvedValue({
			...offerMock,
			restrictions: null
		});
		(OfferService.useOffer as jest.Mock).mockRejectedValue(40056);
		...
		await waitFor(() => {
			expect(getByText('offerDetailsPage.passExpired')).toBeTruthy();
		});
	});
	...
	it('should show generic error message for other API errors', async () => {
		...
		(OfferService.useOffer as jest.Mock).mockRejectedValue(500);
		...
		await waitFor(() => {
			expect(getByText('offerDetailsPage.useOfferError')).toBeTruthy();
		});
	});
});
