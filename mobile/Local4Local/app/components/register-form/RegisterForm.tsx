...
function RegisterForm({ navigation, route }: RegisterFormProps) {
	...
	const isEmptyPasswordFields = (): boolean => {
		return (
			...
			!watch(AuthFormControlsEnum.confirmPassword)
		);
	};
	...
	const onSubmit = (data: any) => {
		...
		createAccount(citizenRegisterDto);
	};

	const createAccount = async (data: CitizenRegisterDto) => {
		try {
			handleCloseError();
			await UserService.registerUser(data);
			setShowDialog(true);
		} catch (error: any) {
			if (error.message == ErrorCode.UniqueConstraintViolated) {
				setUniqueEmail(true);
			} else {
	...
	const passwordFormEntries = [
		...
		},
		{
			...
			placeholder: t(
				"registerPage.registerForm.retypePasswordPlaceholder"
			),
		},
	];
	...
	const handleErrorType = async (error: string) => {
		...
		setRequestError(t("loginPage.requestError"));
	};
		...
		return (
			...
			>
							...
							render={({
								...
								<>
									<TextInput
										...
										error={
											!!errors[
												item.name as keyof GeneralFormData
											]
										}
									/>
				...
				<View style={styles.buttonsContainer}>
					...
					<Text
						variant="bodyLarge"
						style={{ marginBottom: 12, color: colors.BLACK }}
					>
						or
					</Text>
				...
				<View style={styles.bottomContainer}>
					<Text style={styles.linkTextContent}>
						...
						<Text variant="bodyLarge">
							{" "}
							{t("termsAndConditions.textDescription3")}{" "}
						</Text>
						...
						>
							{t("termsAndConditions.textDescription4")}
						</Text>
					</Text>

					<Text style={styles.linkTextBottom}>
						<Text variant="bodyLarge">
							{t("termsAndConditions.question")}{" "}
						</Text>
						<Text
							variant="bodyLarge"
					...
					<View key={item.name + index}>
						<Controller
							...
							rules={{
								...
								...confirmPasswordMatchRule(
									item.name ===
										AuthFormControlsEnum.confirmPassword,
	...
	const genericDialog = () => {
		return (
			<GenericDialog
				...
				onButtonPress={resendConfirmationEmail}
			/>
		);
	};
		...
		<TouchableWithoutFeedback onPress={Keyboard.dismiss} accessible={false}>
			<>
				{!isGeneralStepCompleted ? (
					...
					<>{!showDialog ? passwordStep() : genericDialog()}</>
				)}
			</>
		</TouchableWithoutFeedback>
	);
