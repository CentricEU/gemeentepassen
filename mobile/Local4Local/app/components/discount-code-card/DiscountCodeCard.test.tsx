...
describe("DiscountCodeCard", () => {
	...
	it("renders correctly with discount code", () => {
		...
		expect(getByTestId("discount-code")).toHaveTextContent(
			/A\s*B\s*C\s*1\s*2\s*3/
		);
		expect(getAllByTestId("details-text")[0]).toHaveTextContent(
			"10% discount"
		);
		expect(getAllByTestId("details-text")[1]).toHaveTextContent(
			"offer.expirationDate " + new Date("2023-12-31").toString()
		);
	});
	...
	it("renders correctly with inactive discount code", () => {
		...
		const { getByTestId, getAllByTestId } = render(
			<DiscountCodeCard
				discountCode={inactiveDiscountCode}
				isDiscountsListView={false}
			/>
		);
	...
	it("renders correctly with different offer types", () => {
		...
		offerTypes.forEach((offerType) => {
			...
			const { getAllByTestId } = render(
				<DiscountCodeCard
					discountCode={discountCodeWithOfferType}
					isDiscountsListView={false}
				/>
			);
	...
	it("renders correctly with different expiration dates", () => {
		...
		expirationDates.forEach((expirationDate) => {
			...
			const { getAllByTestId } = render(
				<DiscountCodeCard
					discountCode={discountCodeWithExpirationDate}
					isDiscountsListView={false}
				/>
			);

			expect(getAllByTestId("details-text")[1]).toHaveTextContent(
				"offer.expirationDate " + new Date(expirationDate).toString()
			);
		});
	});
	...
	it("renders correctly with different amounts", () => {
		...
		amounts.forEach((amount) => {
			...
			const { getAllByTestId } = render(
				<DiscountCodeCard
					discountCode={discountCodeWithAmount}
					isDiscountsListView={false}
				/>
			);
	...
	it("renders correctly with different company logos", () => {
		...
		logos.forEach((logo) => {
			...
			const { getByTestId } = render(
				<DiscountCodeCard
					discountCode={discountCodeWithLogo}
					isDiscountsListView={false}
				/>
			);
	...
	it("renders correctly with different company names", () => {
		...
		companyNames.forEach((companyName) => {
			...
			const { getByText } = render(
				<DiscountCodeCard
					discountCode={discountCodeWithCompanyName}
					isDiscountsListView={false}
				/>
			);
