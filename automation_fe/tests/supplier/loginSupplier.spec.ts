...
test.describe("Supplier Successful Login Test", () => {
  test("Successful Login @smoke", async ({ page }) => {
    await loginPage.login(env.EMAIL_SUPPLIER, env.PASSWORD);
    await AssertionHelper.assertPageTitle(page, PAGE_TITLES.SUPPLIER);
...
test.describe("Supplier Login Validation Tests", () => {  
  ...
  test("Unsuccessful login with invalid password", async ( ) => {
    await loginPage.login(env.EMAIL_SUPPLIER, "invalidPassword");
    const toasterMessage = await loginPage.getToasterMessage();
    expect(toasterMessage).toContain(ToasterMessages.INVALID_AUTHENTICATION);
  });
...
test.describe("Supplier Successful Logout Test", () => {  
